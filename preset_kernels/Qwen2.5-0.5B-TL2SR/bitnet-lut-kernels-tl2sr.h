...
#define BITNET_TL2_KERNEL_CONFIG_SHA256 "baeed3c60a3b027e5e7492a58fe7268837eeaf3c05734d67d7fe2e71fd9d47f9"
...
#define BM896_896 128
#define BBK896_896 192
template<int batch_size, int K3>
inline void three_tbl_impl_896_896(int32_t* c, int8_t* lut, uint8_t* a, uint8_t* sign) {
#ifdef __AVX2__
    const __m256i vec_mask = _mm256_set1_epi8(0x0f);
    const int KK = BBK896_896 / 3;
        for (int i = 0; i < BM896_896; i += 32) {
        __m256i vec_as[KK / 2];
    ...
    for (int bs = 0; bs < batch_size; bs++) {
        ...
        __m256i vec_gc0 = _mm256_loadu_si256(reinterpret_cast<__m256i*>(c + i      + BM896_896 * bs));
        __m256i vec_gc1 = _mm256_loadu_si256(reinterpret_cast<__m256i*>(c + i + 8  + BM896_896 * bs));
        __m256i vec_gc2 = _mm256_loadu_si256(reinterpret_cast<__m256i*>(c + i + 16 + BM896_896 * bs));
        __m256i vec_gc3 = _mm256_loadu_si256(reinterpret_cast<__m256i*>(c + i + 24 + BM896_896 * bs));
        ...
        _mm256_storeu_si256(reinterpret_cast<__m256i*>(c + i      + BM896_896 * bs), vec_gc0);
        _mm256_storeu_si256(reinterpret_cast<__m256i*>(c + i + 8  + BM896_896 * bs), vec_gc1);
        _mm256_storeu_si256(reinterpret_cast<__m256i*>(c + i + 16 + BM896_896 * bs), vec_gc2);
        _mm256_storeu_si256(reinterpret_cast<__m256i*>(c + i + 24 + BM896_896 * bs), vec_gc3);
    }
    }
#endif
}
...
inline int32_t two_tbl_impl896_896(int32_t* c, int8_t* lut, uint8_t* a) {
#ifdef __AVX2__
    const __m256i vec_mask = _mm256_set1_epi8(0x0f);
    const int KK = BK2 / 2;
    for (int i = 0; i < BM896_896; i += 32) {
...
int32_t three_qgemm_lut_896_896(void* A, void* sign, void* LUT, void* Scales, int scale_stride, void* LUT_Scales, void* C) {
    (void) Scales;
    (void) scale_stride;
    (void) LUT_Scales;
    alignas(32) uint32_t CBits[BATCH_SIZE * BM896_896];
    memset(&(CBits[0]), 0, BATCH_SIZE * BM896_896 * sizeof(int32_t));
    for (int32_t k_outer = 0; k_outer < 768 / BBK896_896; ++k_outer) {
        three_tbl_impl_896_896<BATCH_SIZE, 768>((&(((int32_t*)CBits)[0])), (&(((int8_t*)LUT)[(k_outer * BBK896_896 / 3 * 32)])), (&(((uint8_t*)A)[(k_outer * BBK896_896 / 3 / 2 * BM896_896)])), (&(((uint8_t*)sign)[(k_outer * BBK896_896 / 3 / 8 * BM896_896)])));
    }
    for (int bs = 0; bs < BATCH_SIZE; bs++) {
        for (int i = 0; i < BM896_896; i++) {
            ((int32_t*)C)[i + bs * 896] = (int32_t)(((int32_t*)CBits)[i + bs * BM896_896]);
        }
  }
  return 0;
}
...
template<int BATCH_SIZE>
int32_t two_qgemm_lut_896_896(void* A, void* LUT, void* Scales, int scale_stride, void* LUT_Scales, void* C) {
    alignas(32) uint32_t CBits[BATCH_SIZE * BM896_896];
    memset(&(CBits[0]), 0, BATCH_SIZE * BM896_896 * sizeof(int32_t));
    for (int32_t k_outer = 0; k_outer < 128 / 32; ++k_outer) {
        two_tbl_impl896_896<BATCH_SIZE, 128>((&(((int32_t*)CBits)[0])), (&(((int8_t*)LUT)[(k_outer * BK2 / 2 * 32)])), (&(((uint8_t*)A)[(k_outer * BK2 / 2 / 2 * BM896_896)])));
    }
    for (int bs = 0; bs < BATCH_SIZE; bs++) {
        for (int i = 0; i < BM896_896; i++) {
            const int output_idx = i + bs * 896;
            ((int32_t*)C)[output_idx] += (int32_t)(((int32_t*)CBits)[i + bs * BM896_896]);
...
#define BM128_896 128
#define BBK128_896 192
template<int batch_size, int K3>
inline void three_tbl_impl_128_896(int32_t* c, int8_t* lut, uint8_t* a, uint8_t* sign) {
#ifdef __AVX2__
    const __m256i vec_mask = _mm256_set1_epi8(0x0f);
    const int KK = BBK128_896 / 3;
        for (int i = 0; i < BM128_896; i += 32) {
        __m256i vec_as[KK / 2];
        __m256i vec_signs[KK / 8];
        for (int ai = 0; ai < KK / 2; ai++) {
            vec_as[ai] = _mm256_loadu_si256(reinterpret_cast<__m256i*>(a + i * KK / 2 + ai * 32));
    ...
    for (int bs = 0; bs < batch_size; bs++) {
        ...
        __m256i vec_gc0 = _mm256_loadu_si256(reinterpret_cast<__m256i*>(c + i      + BM128_896 * bs));
        __m256i vec_gc1 = _mm256_loadu_si256(reinterpret_cast<__m256i*>(c + i + 8  + BM128_896 * bs));
        __m256i vec_gc2 = _mm256_loadu_si256(reinterpret_cast<__m256i*>(c + i + 16 + BM128_896 * bs));
        __m256i vec_gc3 = _mm256_loadu_si256(reinterpret_cast<__m256i*>(c + i + 24 + BM128_896 * bs));
        vec_gc0 = _mm256_add_epi32(vec_gc0, _mm256_cvtepi16_epi32(_mm256_castsi256_si128(vec_c0)));
        vec_gc1 = _mm256_add_epi32(vec_gc1, _mm256_cvtepi16_epi32(_mm256_extracti128_si256(vec_c0, 1)));
        vec_gc2 = _mm256_add_epi32(vec_gc2, _mm256_cvtepi16_epi32(_mm256_castsi256_si128(vec_c1)));
        vec_gc3 = _mm256_add_epi32(vec_gc3, _mm256_cvtepi16_epi32(_mm256_extracti128_si256(vec_c1, 1)));
        _mm256_storeu_si256(reinterpret_cast<__m256i*>(c + i      + BM128_896 * bs), vec_gc0);
        _mm256_storeu_si256(reinterpret_cast<__m256i*>(c + i + 8  + BM128_896 * bs), vec_gc1);
        _mm256_storeu_si256(reinterpret_cast<__m256i*>(c + i + 16 + BM128_896 * bs), vec_gc2);
        _mm256_storeu_si256(reinterpret_cast<__m256i*>(c + i + 24 + BM128_896 * bs), vec_gc3);
    }
    }
#endif
}
...
template<int batch_size, int K2>
inline int32_t two_tbl_impl128_896(int32_t* c, int8_t* lut, uint8_t* a) {
#ifdef __AVX2__
    const __m256i vec_mask = _mm256_set1_epi8(0x0f);
    const int KK = BK2 / 2;
    for (int i = 0; i < BM128_896; i += 32) {
...
template<int BATCH_SIZE>
int32_t three_qgemm_lut_128_896(void* A, void* sign, void* LUT, void* Scales, int scale_stride, void* LUT_Scales, void* C) {
    (void) Scales;
    (void) scale_stride;
    (void) LUT_Scales;
    alignas(32) uint32_t CBits[BATCH_SIZE * BM128_896];
    memset(&(CBits[0]), 0, BATCH_SIZE * BM128_896 * sizeof(int32_t));
    for (int32_t k_outer = 0; k_outer < 768 / BBK128_896; ++k_outer) {
        three_tbl_impl_128_896<BATCH_SIZE, 768>((&(((int32_t*)CBits)[0])), (&(((int8_t*)LUT)[(k_outer * BBK128_896 / 3 * 32)])), (&(((uint8_t*)A)[(k_outer * BBK128_896 / 3 / 2 * BM128_896)])), (&(((uint8_t*)sign)[(k_outer * BBK128_896 / 3 / 8 * BM128_896)])));
    }
    for (int bs = 0; bs < BATCH_SIZE; bs++) {
        for (int i = 0; i < BM128_896; i++) {
            ((int32_t*)C)[i + bs * 128] = (int32_t)(((int32_t*)CBits)[i + bs * BM128_896]);
        }
  }
  return 0;
}
...
template<int BATCH_SIZE>
int32_t two_qgemm_lut_128_896(void* A, void* LUT, void* Scales, int scale_stride, void* LUT_Scales, void* C) {
    alignas(32) uint32_t CBits[BATCH_SIZE * BM128_896];
    memset(&(CBits[0]), 0, BATCH_SIZE * BM128_896 * sizeof(int32_t));
    for (int32_t k_outer = 0; k_outer < 128 / 32; ++k_outer) {
        two_tbl_impl128_896<BATCH_SIZE, 128>((&(((int32_t*)CBits)[0])), (&(((int8_t*)LUT)[(k_outer * BK2 / 2 * 32)])), (&(((uint8_t*)A)[(k_outer * BK2 / 2 / 2 * BM128_896)])));
    }
    for (int bs = 0; bs < BATCH_SIZE; bs++) {
        for (int i = 0; i < BM128_896; i++) {
            const int output_idx = i + bs * 128;
            ((int32_t*)C)[output_idx] += (int32_t)(((int32_t*)CBits)[i + bs * BM128_896]);
            ((float*)C)[output_idx] = (float)(((int32_t*)C)[output_idx]) / ((float*)LUT_Scales)[bs] * ((float*)Scales)[i * scale_stride];
        }
    }
  return 0;
...
#define BM4864_896 128
#define BBK4864_896 192
template<int batch_size, int K3>
inline void three_tbl_impl_4864_896(int32_t* c, int8_t* lut, uint8_t* a, uint8_t* sign) {
#ifdef __AVX2__
    const __m256i vec_mask = _mm256_set1_epi8(0x0f);
    const int KK = BBK4864_896 / 3;
        for (int i = 0; i < BM4864_896; i += 32) {
        __m256i vec_as[KK / 2];
        __m256i vec_signs[KK / 8];
        for (int ai = 0; ai < KK / 2; ai++) {
            vec_as[ai] = _mm256_loadu_si256(reinterpret_cast<__m256i*>(a + i * KK / 2 + ai * 32));
    ...
    for (int bs = 0; bs < batch_size; bs++) {
        ...
        __m256i vec_gc0 = _mm256_loadu_si256(reinterpret_cast<__m256i*>(c + i      + BM4864_896 * bs));
        __m256i vec_gc1 = _mm256_loadu_si256(reinterpret_cast<__m256i*>(c + i + 8  + BM4864_896 * bs));
        __m256i vec_gc2 = _mm256_loadu_si256(reinterpret_cast<__m256i*>(c + i + 16 + BM4864_896 * bs));
        __m256i vec_gc3 = _mm256_loadu_si256(reinterpret_cast<__m256i*>(c + i + 24 + BM4864_896 * bs));
        vec_gc0 = _mm256_add_epi32(vec_gc0, _mm256_cvtepi16_epi32(_mm256_castsi256_si128(vec_c0)));
        vec_gc1 = _mm256_add_epi32(vec_gc1, _mm256_cvtepi16_epi32(_mm256_extracti128_si256(vec_c0, 1)));
        vec_gc2 = _mm256_add_epi32(vec_gc2, _mm256_cvtepi16_epi32(_mm256_castsi256_si128(vec_c1)));
        vec_gc3 = _mm256_add_epi32(vec_gc3, _mm256_cvtepi16_epi32(_mm256_extracti128_si256(vec_c1, 1)));
        _mm256_storeu_si256(reinterpret_cast<__m256i*>(c + i      + BM4864_896 * bs), vec_gc0);
        _mm256_storeu_si256(reinterpret_cast<__m256i*>(c + i + 8  + BM4864_896 * bs), vec_gc1);
        _mm256_storeu_si256(reinterpret_cast<__m256i*>(c + i + 16 + BM4864_896 * bs), vec_gc2);
        _mm256_storeu_si256(reinterpret_cast<__m256i*>(c + i + 24 + BM4864_896 * bs), vec_gc3);
    }
    }
#endif
}
...
template<int batch_size, int K2>
inline int32_t two_tbl_impl4864_896(int32_t* c, int8_t* lut, uint8_t* a) {
#ifdef __AVX2__
    const __m256i vec_mask = _mm256_set1_epi8(0x0f);
    const int KK = BK2 / 2;
    for (int i = 0; i < BM4864_896; i += 32) {
...
template<int BATCH_SIZE>
int32_t three_qgemm_lut_4864_896(void* A, void* sign, void* LUT, void* Scales, int scale_stride, void* LUT_Scales, void* C) {
    (void) Scales;
    (void) scale_stride;
    (void) LUT_Scales;
    alignas(32) uint32_t CBits[BATCH_SIZE * BM4864_896];
    memset(&(CBits[0]), 0, BATCH_SIZE * BM4864_896 * sizeof(int32_t));
    for (int32_t k_outer = 0; k_outer < 768 / BBK4864_896; ++k_outer) {
        three_tbl_impl_4864_896<BATCH_SIZE, 768>((&(((int32_t*)CBits)[0])), (&(((int8_t*)LUT)[(k_outer * BBK4864_896 / 3 * 32)])), (&(((uint8_t*)A)[(k_outer * BBK4864_896 / 3 / 2 * BM4864_896)])), (&(((uint8_t*)sign)[(k_outer * BBK4864_896 / 3 / 8 * BM4864_896)])));
    }
    for (int bs = 0; bs < BATCH_SIZE; bs++) {
        for (int i = 0; i < BM4864_896; i++) {
            ((int32_t*)C)[i + bs * 4864] = (int32_t)(((int32_t*)CBits)[i + bs * BM4864_896]);
        }
  }
  return 0;
}
...
template<int BATCH_SIZE>
int32_t two_qgemm_lut_4864_896(void* A, void* LUT, void* Scales, int scale_stride, void* LUT_Scales, void* C) {
    alignas(32) uint32_t CBits[BATCH_SIZE * BM4864_896];
    memset(&(CBits[0]), 0, BATCH_SIZE * BM4864_896 * sizeof(int32_t));
    for (int32_t k_outer = 0; k_outer < 128 / 32; ++k_outer) {
        two_tbl_impl4864_896<BATCH_SIZE, 128>((&(((int32_t*)CBits)[0])), (&(((int8_t*)LUT)[(k_outer * BK2 / 2 * 32)])), (&(((uint8_t*)A)[(k_outer * BK2 / 2 / 2 * BM4864_896)])));
    }
    for (int bs = 0; bs < BATCH_SIZE; bs++) {
        for (int i = 0; i < BM4864_896; i++) {
            const int output_idx = i + bs * 4864;
            ((int32_t*)C)[output_idx] += (int32_t)(((int32_t*)CBits)[i + bs * BM4864_896]);
            ((float*)C)[output_idx] = (float)(((int32_t*)C)[output_idx]) / ((float*)LUT_Scales)[bs] * ((float*)Scales)[i * scale_stride];
        }
    }
  return 0;
...
#define BM896_4864 128
#define BBK896_4864 192
template<int batch_size, int K3>
inline void three_tbl_impl_896_4864(int32_t* c, int8_t* lut, uint8_t* a, uint8_t* sign) {
#ifdef __AVX2__
    const __m256i vec_mask = _mm256_set1_epi8(0x0f);
    const int KK = BBK896_4864 / 3;
        for (int i = 0; i < BM896_4864; i += 32) {
        __m256i vec_as[KK / 2];
        __m256i vec_signs[KK / 8];
        for (int ai = 0; ai < KK / 2; ai++) {
            vec_as[ai] = _mm256_loadu_si256(reinterpret_cast<__m256i*>(a + i * KK / 2 + ai * 32));
    ...
    for (int bs = 0; bs < batch_size; bs++) {
        ...
        __m256i vec_gc0 = _mm256_loadu_si256(reinterpret_cast<__m256i*>(c + i      + BM896_4864 * bs));
        __m256i vec_gc1 = _mm256_loadu_si256(reinterpret_cast<__m256i*>(c + i + 8  + BM896_4864 * bs));
        __m256i vec_gc2 = _mm256_loadu_si256(reinterpret_cast<__m256i*>(c + i + 16 + BM896_4864 * bs));
        __m256i vec_gc3 = _mm256_loadu_si256(reinterpret_cast<__m256i*>(c + i + 24 + BM896_4864 * bs));
        vec_gc0 = _mm256_add_epi32(vec_gc0, _mm256_cvtepi16_epi32(_mm256_castsi256_si128(vec_c0)));
        vec_gc1 = _mm256_add_epi32(vec_gc1, _mm256_cvtepi16_epi32(_mm256_extracti128_si256(vec_c0, 1)));
        vec_gc2 = _mm256_add_epi32(vec_gc2, _mm256_cvtepi16_epi32(_mm256_castsi256_si128(vec_c1)));
        vec_gc3 = _mm256_add_epi32(vec_gc3, _mm256_cvtepi16_epi32(_mm256_extracti128_si256(vec_c1, 1)));
        _mm256_storeu_si256(reinterpret_cast<__m256i*>(c + i      + BM896_4864 * bs), vec_gc0);
        _mm256_storeu_si256(reinterpret_cast<__m256i*>(c + i + 8  + BM896_4864 * bs), vec_gc1);
        _mm256_storeu_si256(reinterpret_cast<__m256i*>(c + i + 16 + BM896_4864 * bs), vec_gc2);
        _mm256_storeu_si256(reinterpret_cast<__m256i*>(c + i + 24 + BM896_4864 * bs), vec_gc3);
    }
    }
#endif
}
...
template<int batch_size, int K2>
inline int32_t two_tbl_impl896_4864(int32_t* c, int8_t* lut, uint8_t* a) {
#ifdef __AVX2__
    const __m256i vec_mask = _mm256_set1_epi8(0x0f);
    const int KK = BK2 / 2;
    for (int i = 0; i < BM896_4864; i += 32) {
...
template<int BATCH_SIZE>
int32_t three_qgemm_lut_896_4864(void* A, void* sign, void* LUT, void* Scales, int scale_stride, void* LUT_Scales, void* C) {
    (void) Scales;
    (void) scale_stride;
    (void) LUT_Scales;
    alignas(32) uint32_t CBits[BATCH_SIZE * BM896_4864];
    memset(&(CBits[0]), 0, BATCH_SIZE * BM896_4864 * sizeof(int32_t));
    for (int32_t k_outer = 0; k_outer < 4800 / BBK896_4864; ++k_outer) {
        three_tbl_impl_896_4864<BATCH_SIZE, 4800>((&(((int32_t*)CBits)[0])), (&(((int8_t*)LUT)[(k_outer * BBK896_4864 / 3 * 32)])), (&(((uint8_t*)A)[(k_outer * BBK896_4864 / 3 / 2 * BM896_4864)])), (&(((uint8_t*)sign)[(k_outer * BBK896_4864 / 3 / 8 * BM896_4864)])));
    }
    for (int bs = 0; bs < BATCH_SIZE; bs++) {
        for (int i = 0; i < BM896_4864; i++) {
            ((int32_t*)C)[i + bs * 896] = (int32_t)(((int32_t*)CBits)[i + bs * BM896_4864]);
        }
  }
  return 0;
}
...
template<int BATCH_SIZE>
int32_t two_qgemm_lut_896_4864(void* A, void* LUT, void* Scales, int scale_stride, void* LUT_Scales, void* C) {
    alignas(32) uint32_t CBits[BATCH_SIZE * BM896_4864];
    memset(&(CBits[0]), 0, BATCH_SIZE * BM896_4864 * sizeof(int32_t));
    for (int32_t k_outer = 0; k_outer < 64 / 32; ++k_outer) {
        two_tbl_impl896_4864<BATCH_SIZE, 64>((&(((int32_t*)CBits)[0])), (&(((int8_t*)LUT)[(k_outer * BK2 / 2 * 32)])), (&(((uint8_t*)A)[(k_outer * BK2 / 2 / 2 * BM896_4864)])));
    }
    for (int bs = 0; bs < BATCH_SIZE; bs++) {
        for (int i = 0; i < BM896_4864; i++) {
            const int output_idx = i + bs * 896;
            ((int32_t*)C)[output_idx] += (int32_t)(((int32_t*)CBits)[i + bs * BM896_4864]);
            ((float*)C)[output_idx] = (float)(((int32_t*)C)[output_idx]) / ((float*)LUT_Scales)[bs] * ((float*)Scales)[i * scale_stride];
        }
    }
  return 0;
...
void ggml_preprocessor(int bs, int m, int three_k, int two_k, void* B, void* LUT_Scales, void* Three_QLUT, void* Two_QLUT) {
    ...
    if (m == 896 && two_k == 128 && three_k == 768) {
        for (int32_t b = 0; b < bs; b++) {
            ...
            two_lut_ctor<128>((&(((int8_t*)Two_QLUT)[b * two_k / 2 * 32])), (&(((float*)B)[b * (three_k + two_k) + 768])), (&(((float*)LUT_Scales)[b])));
        }
    }
    else if (m == 128 && two_k == 128 && three_k == 768) {
        for (int32_t b = 0; b < bs; b++) {
            per_tensor_quant(two_k + three_k, (&(((float*)LUT_Scales)[b])), (&(((float*)B)[b * (two_k + three_k)])));
            three_lut_ctor<768>((&(((int8_t*)Three_QLUT)[b * three_k / 3 * 32])), (&(((float*)B)[b * (three_k + two_k)])), (&(((float*)LUT_Scales)[b])));
            two_lut_ctor<128>((&(((int8_t*)Two_QLUT)[b * two_k / 2 * 32])), (&(((float*)B)[b * (three_k + two_k) + 768])), (&(((float*)LUT_Scales)[b])));
    ...
    else if (m == 4864 && two_k == 128 && three_k == 768) {
        for (int32_t b = 0; b < bs; b++) {
            per_tensor_quant(two_k + three_k, (&(((float*)LUT_Scales)[b])), (&(((float*)B)[b * (two_k + three_k)])));
            three_lut_ctor<768>((&(((int8_t*)Three_QLUT)[b * three_k / 3 * 32])), (&(((float*)B)[b * (three_k + two_k)])), (&(((float*)LUT_Scales)[b])));
            two_lut_ctor<128>((&(((int8_t*)Two_QLUT)[b * two_k / 2 * 32])), (&(((float*)B)[b * (three_k + two_k) + 768])), (&(((float*)LUT_Scales)[b])));
    ...
    else if (m == 896 && two_k == 64 && three_k == 4800) {
        for (int32_t b = 0; b < bs; b++) {
            per_tensor_quant(two_k + three_k, (&(((float*)LUT_Scales)[b])), (&(((float*)B)[b * (two_k + three_k)])));
            three_lut_ctor<4800>((&(((int8_t*)Three_QLUT)[b * three_k / 3 * 32])), (&(((float*)B)[b * (three_k + two_k)])), (&(((float*)LUT_Scales)[b])));
            two_lut_ctor<64>((&(((int8_t*)Two_QLUT)[b * two_k / 2 * 32])), (&(((float*)B)[b * (three_k + two_k) + 4800])), (&(((float*)LUT_Scales)[b])));
        }
    }
}
void ggml_qgemm_lut(int bs, int m, int k, int BK, void* A, void* sign, void* LUT, void* Scales, int scale_stride, void* LUT_Scales, void* C) {
    if (m == 896 && k == 896) {
        if (BK == 128) {
            if (bs == 1) {
                two_qgemm_lut_896_896<1>(A, LUT, Scales, scale_stride, LUT_Scales, C);
            } else if (bs == 8) {
                two_qgemm_lut_896_896<8>(A, LUT, Scales, scale_stride, LUT_Scales, C);
            } else if (bs == 32) {
                two_qgemm_lut_896_896<32>(A, LUT, Scales, scale_stride, LUT_Scales, C);
            } else if (bs == 128) {
                two_qgemm_lut_896_896<128>(A, LUT, Scales, scale_stride, LUT_Scales, C);
            } else if (bs == 256) {
                two_qgemm_lut_896_896<256>(A, LUT, Scales, scale_stride, LUT_Scales, C);
            } else if (bs == 512) {
                two_qgemm_lut_896_896<512>(A, LUT, Scales, scale_stride, LUT_Scales, C);
            }
        }
        else if (BK == 768) {
            if (bs == 1) {
                three_qgemm_lut_896_896<1>(A, sign, LUT, Scales, scale_stride, LUT_Scales, C);
            }else if (bs == 8) {
                three_qgemm_lut_896_896<8>(A, sign, LUT, Scales, scale_stride, LUT_Scales, C);
            }else if (bs == 32) {
                three_qgemm_lut_896_896<32>(A, sign, LUT, Scales, scale_stride, LUT_Scales, C);
            }else if (bs == 128) {
                three_qgemm_lut_896_896<128>(A, sign, LUT, Scales, scale_stride, LUT_Scales, C);
            }else if (bs == 256) {
                three_qgemm_lut_896_896<256>(A, sign, LUT, Scales, scale_stride, LUT_Scales, C);
            }else if (bs == 512) {
                three_qgemm_lut_896_896<512>(A, sign, LUT, Scales, scale_stride, LUT_Scales, C);
            }
        }
    }
    else if (m == 128 && k == 896) {
        if (BK == 128) {
            if (bs == 1) {
                two_qgemm_lut_128_896<1>(A, LUT, Scales, scale_stride, LUT_Scales, C);
            } else if (bs == 8) {
                two_qgemm_lut_128_896<8>(A, LUT, Scales, scale_stride, LUT_Scales, C);
            } else if (bs == 32) {
                two_qgemm_lut_128_896<32>(A, LUT, Scales, scale_stride, LUT_Scales, C);
            } else if (bs == 128) {
                two_qgemm_lut_128_896<128>(A, LUT, Scales, scale_stride, LUT_Scales, C);
            } else if (bs == 256) {
                two_qgemm_lut_128_896<256>(A, LUT, Scales, scale_stride, LUT_Scales, C);
            } else if (bs == 512) {
                two_qgemm_lut_128_896<512>(A, LUT, Scales, scale_stride, LUT_Scales, C);
            }
        }
        else if (BK == 768) {
            if (bs == 1) {
                three_qgemm_lut_128_896<1>(A, sign, LUT, Scales, scale_stride, LUT_Scales, C);
            }else if (bs == 8) {
                three_qgemm_lut_128_896<8>(A, sign, LUT, Scales, scale_stride, LUT_Scales, C);
            }else if (bs == 32) {
                three_qgemm_lut_128_896<32>(A, sign, LUT, Scales, scale_stride, LUT_Scales, C);
            }else if (bs == 128) {
                three_qgemm_lut_128_896<128>(A, sign, LUT, Scales, scale_stride, LUT_Scales, C);
            }else if (bs == 256) {
                three_qgemm_lut_128_896<256>(A, sign, LUT, Scales, scale_stride, LUT_Scales, C);
            }else if (bs == 512) {
                three_qgemm_lut_128_896<512>(A, sign, LUT, Scales, scale_stride, LUT_Scales, C);
            }
        }
    }
    else if (m == 4864 && k == 896) {
        if (BK == 128) {
            if (bs == 1) {
                two_qgemm_lut_4864_896<1>(A, LUT, Scales, scale_stride, LUT_Scales, C);
            } else if (bs == 8) {
                two_qgemm_lut_4864_896<8>(A, LUT, Scales, scale_stride, LUT_Scales, C);
            } else if (bs == 32) {
                two_qgemm_lut_4864_896<32>(A, LUT, Scales, scale_stride, LUT_Scales, C);
            } else if (bs == 128) {
                two_qgemm_lut_4864_896<128>(A, LUT, Scales, scale_stride, LUT_Scales, C);
            } else if (bs == 256) {
                two_qgemm_lut_4864_896<256>(A, LUT, Scales, scale_stride, LUT_Scales, C);
            } else if (bs == 512) {
                two_qgemm_lut_4864_896<512>(A, LUT, Scales, scale_stride, LUT_Scales, C);
            }
        }
        else if (BK == 768) {
            if (bs == 1) {
                three_qgemm_lut_4864_896<1>(A, sign, LUT, Scales, scale_stride, LUT_Scales, C);
            }else if (bs == 8) {
                three_qgemm_lut_4864_896<8>(A, sign, LUT, Scales, scale_stride, LUT_Scales, C);
            }else if (bs == 32) {
                three_qgemm_lut_4864_896<32>(A, sign, LUT, Scales, scale_stride, LUT_Scales, C);
            }else if (bs == 128) {
                three_qgemm_lut_4864_896<128>(A, sign, LUT, Scales, scale_stride, LUT_Scales, C);
            }else if (bs == 256) {
                three_qgemm_lut_4864_896<256>(A, sign, LUT, Scales, scale_stride, LUT_Scales, C);
            }else if (bs == 512) {
                three_qgemm_lut_4864_896<512>(A, sign, LUT, Scales, scale_stride, LUT_Scales, C);
            }
        }
    }
    else if (m == 896 && k == 4864) {
        if (BK == 64) {
            if (bs == 1) {
                two_qgemm_lut_896_4864<1>(A, LUT, Scales, scale_stride, LUT_Scales, C);
            } else if (bs == 8) {
                two_qgemm_lut_896_4864<8>(A, LUT, Scales, scale_stride, LUT_Scales, C);
            } else if (bs == 32) {
                two_qgemm_lut_896_4864<32>(A, LUT, Scales, scale_stride, LUT_Scales, C);
            } else if (bs == 128) {
                two_qgemm_lut_896_4864<128>(A, LUT, Scales, scale_stride, LUT_Scales, C);
            } else if (bs == 256) {
                two_qgemm_lut_896_4864<256>(A, LUT, Scales, scale_stride, LUT_Scales, C);
            } else if (bs == 512) {
                two_qgemm_lut_896_4864<512>(A, LUT, Scales, scale_stride, LUT_Scales, C);
            }
        }
        else if (BK == 4800) {
            if (bs == 1) {
                three_qgemm_lut_896_4864<1>(A, sign, LUT, Scales, scale_stride, LUT_Scales, C);
            }else if (bs == 8) {
                three_qgemm_lut_896_4864<8>(A, sign, LUT, Scales, scale_stride, LUT_Scales, C);
            }else if (bs == 32) {
                three_qgemm_lut_896_4864<32>(A, sign, LUT, Scales, scale_stride, LUT_Scales, C);
            }else if (bs == 128) {
                three_qgemm_lut_896_4864<128>(A, sign, LUT, Scales, scale_stride, LUT_Scales, C);
            }else if (bs == 256) {
                three_qgemm_lut_896_4864<256>(A, sign, LUT, Scales, scale_stride, LUT_Scales, C);
            }else if (bs == 512) {
                three_qgemm_lut_896_4864<512>(A, sign, LUT, Scales, scale_stride, LUT_Scales, C);
            }
        }
    }
}
