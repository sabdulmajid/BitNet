...
| BitDistill telemetry coverage audit measures current loss diagnostics | pass | status=partial_observability, measured=9/9 |  |
| BitDistill telemetry coverage audit keeps advanced causality claims blocked | pass | missing=['gradient norm by loss component', 'scale trajectory per layer', 'ternary flip rate per step/layer'], measured_activation=True |  |
