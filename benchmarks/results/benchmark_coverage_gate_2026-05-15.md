...
| BitDistill telemetry coverage audit measures current loss diagnostics | pass | status=partial_observability, measured=8/8 |  |
| BitDistill telemetry coverage audit keeps advanced causality claims blocked | pass | missing=['activation int8 saturation rate', 'gradient norm by loss component', 'scale trajectory per layer', 'ternary flip rate per step/layer'] |  |
