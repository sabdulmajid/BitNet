...
Scenario: `I2_SR split-patch promotion rehearsal`.

Note: Generated by benchmarks/run_i2sr_promotion_rehearsal.py after applying patches/bitnet-i2sr-root-runtime.patch in the superproject and patches/llama-i2sr-row-scale-qtype.submodule.patch inside 3rdparty/llama.cpp; patched build targets llama-cli,llama-bench,llama-perplexity,llama-quantize were rebuilt; the script reverses both patches before exiting; default build targets llama-cli,llama-bench,llama-perplexity,llama-quantize were rebuilt after patch reversal.
