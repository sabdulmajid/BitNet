...
- The Qwen3 SST2 branch now has full-validation FP16, BitNet-SFT, tensor
  BitDistill, and row BitDistill rows: FP16-SFT `0.930046`, BitNet-SFT
  `0.799312`, tensor BitDistill `0.871560`, and row BitDistill `0.877294`.
  Tensor BitDistill improves over BitNet-SFT by `+0.072248`, but the paired
  delta versus FP remains `-0.058486` with 95% CI
  `[-0.080523, -0.036449]`. Row BitDistill is slightly higher than tensor, but
  the paired row-minus-tensor CI crosses zero: `+0.005734`
  `[-0.011536, 0.023004]`. This is useful recovery, not a reproduction pass.
- The first Qwen3 MNLI attention-layer sweep confirms layer selection matters:
  layer `-8` reaches `0.752012` versus the layer `-1` tensor baseline
  `0.723484`, paired delta `+0.028528` with 95% CI
  `[0.022008, 0.035048]`. It still trails FP16-SFT `0.829750` by paired delta
  `-0.077738`, so this narrows but does not close the Qwen3 gap.
...
checkpoint exports as a packed `I2_SR` decoder backbone plus dense score-head
...
audited MNLI sidecar sample, token IDs match, hidden cosine is `0.994091`,
hidden relative RMS is `0.108662`, and score-logit relative RMS is `0.091918`.
