...
| 5. Convert repaired checkpoints into GGUF/TL2/I2_S and run CPU inference | partial | direct dense/scalar writers exist; I2_SR candidate gate=True; active default gate=True; TL2 row-scale one-scale error=1.9042302114103853; row-fp16 design error=0.00019744640689756221 at 1.23046875 MiB; TL2 ready=False; TL2 current row-scale error=1.9042302114103853; TL2 failed checks=6 | Dense GGUF and row-scale I2_SR/I2_S CPU inference exist, but TL2 is not quality-preserving for learned row-scale checkpoints until row/group-scale metadata and kernels are implemented. |
