...
| CPU producer depends on all downstream jobs with afterany | pass | job=10025, deps=38/38, dependency=afterany:9943(unfulfilled),afterany:9944(unfulfilled),afterany:9945(unfulfilled) |  |
...
| exactly one active I2_SR producer | pass | jobs=[{'job_id': '10024', 'name': 'bitdistill-i2sr', 'state': 'PENDING', 'reason': '(Dependency)'}] |  |
| I2_SR producer stored script matches current script | pass | job=10024, stored=dae3cdd963ec, current=dae3cdd963ec |  |
...
| postprocess finalizers depend on current CPU producer | pass | cpu=10025, strict=['9993', '9994', '9995', '10024', '10025'], afterany=['9993', '9994', '9995', '10024', '10025'] |  |
| current postprocess script includes task-formulation audit | pass | slurm_bitdistill_postprocess.sh |  |
| latest strict postprocess stored script matches current script | pass | job=10027, stored=3b309e8daea5, current=3b309e8daea5 |  |
| latest afterany postprocess stored script matches current script | pass | job=10026, stored=3b309e8daea5, current=3b309e8daea5 |  |
