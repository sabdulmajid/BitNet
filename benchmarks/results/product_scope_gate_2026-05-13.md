...
| TL2 product support for the strong row-scale Qwen checkpoint | unsupported | Qwen0.5B TL2 PPL=NaN; Qwen1.5B row-scale one-scale error=1.9042302114103853 | Exclude TL2 from MVP claims. | Current TL2 scale semantics are incompatible with row-scale checkpoint quality and the tested TL2 artifact has NaN quality. |
| MoE/Kimi retrofit and CPU runtime support | unsupported | Kimi artifacts=0; Kimi source matches=0 | Treat as separate research milestone. | No Kimi-specific converter/runtime mapping, router distillation, quality benchmark, or expert-locality benchmark exists. |
