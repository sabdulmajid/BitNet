...
| Row-scale I2_S/I2_SR quality preservation | complete as prototype/candidate | heap-fix confirmation `benchmark_results/gguf-qwen15b-row-i2s-heapfix-confirm/summary.json`; fixed I2_SR suite `benchmark_results/i2sr-row-scale-qwen15b-x86act-suite-2026-05-13/summary.json`; byte-layout verifier `benchmarks/results/i2s_packing_layout_verify_2026-05-13.md`; audit `benchmark_results/evidence_audit/qwen15b_row_i2s_heapfix.md`; format audit `benchmarks/results/i2s_row_scale_format_audit_2026-05-13.md`; active-source qtype gate `benchmarks/results/row_scale_qtype_productization_gate_2026-05-13.md`; candidate active-patch gate `benchmarks/results/row_scale_qtype_productization_gate_i2sr_active_patch_2026-05-13.md` | not an upstream/default stable GGUF format; fixed `I2_SR` still requires a downstream patch, though that patch passes all nine productization gates when applied |
...
| Packed GGUF RSS | complete for current row-scale suite | `benchmarks/results/gguf_memory_2026-05-05.md`; `benchmarks/results/gguf_context_scaling_2026-05-05.md`; fixed `I2_SR` RSS note `benchmarks/results/i2sr_rss_2026-05-13.md`; audits `benchmark_results/evidence_audit/qwen15b_row_i2s_rss.md` and `benchmark_results/evidence_audit/qwen15b_context_scaling_rss.md` | none for dense Qwen2.5-1.5B GGUF RSS; still not MoE/TL2 |
| Side-by-side comparison and coverage gate | complete for current artifacts | `benchmarks/results/qwen_side_by_side_2026-05-05.md`; generated by `benchmarks/build_qwen_side_by_side.py`; coverage gate `benchmarks/results/benchmark_coverage_gate_2026-05-13.md` passes the ten-task, paired-delta, CPU-row, RSS-context, manifest, and productization-gate checks | does not cover unfinished TL2/MoE paths |
...
1. Promote the fixed `I2_SR` GGUF type into the active runtime contract. The
   ...
   checks. Applying `patches/llama-i2sr-row-scale-qtype.patch` makes the
   productization gate pass `9/9`, but the committed submodule still distributes
   that support as a patch rather than as the active default runtime contract.
