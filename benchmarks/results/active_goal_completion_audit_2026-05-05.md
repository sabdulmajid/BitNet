...
| GGUF conversion and packed CPU inference | partial | static-ternary materialization, TQ2_0, tensor-scale I2_S, row-scale I2_S prototype; `patches/llama-i2s-row-scale.patch` | direct `ternary_state_dict.pt` GGUF writer is not complete |
| TL2 conversion and CPU inference | partial | code-generation exploration and documented limitation in `benchmarks/results/progress_audit_2026-05-05.md` | Qwen-aware TL2 writer/loader path not complete |
...
| Packed GGUF RSS | complete for current row-scale suite | `benchmarks/results/gguf_memory_2026-05-05.md`; audit `benchmark_results/evidence_audit/qwen15b_row_i2s_rss.md` | context-length scaling not yet profiled |
...
| MoE/Kimi feasibility | not complete | codebase notes identify generic MoE support in vendored llama.cpp | no Kimi converter, router distillation, MoE quality run, or expert-locality benchmark |
