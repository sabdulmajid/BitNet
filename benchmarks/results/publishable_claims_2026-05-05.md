...
| Applying the downstream `I2_SR` patch satisfies the row-scale qtype productization gate for that patched source state | supported as candidate activation proof | `benchmarks/results/row_scale_qtype_productization_gate_i2sr_active_patch_2026-05-13.md`; `benchmark_results/row_scale_qtype_productization_gate_i2sr_active_patch_2026-05-13.json`; `patches/llama-i2sr-row-scale-qtype.patch` | this is not the committed default submodule state; it proves the patch contract passes all nine productization checks when applied |
