...
| Direct packed row-scale GGUF export is still unsupported | supported | `benchmarks/results/direct_packed_gguf_support_2026-05-13.md`; `benchmark_results/direct_packed_gguf_support_2026-05-13.json`; `benchmarks/results/direct_i2s_scalar_gguf_2026-05-13.md` | scalar direct `I2_S` exists, but row-scale needs a stable type/version and is intentionally rejected by the direct scalar writer |
...
| Direct packed `ternary_state_dict.pt` to CPU-native GGUF writing is complete | unsupported | direct dense F16 GGUF and direct scalar `I2_S` now work, but the scalar Qwen0.5B quality result is NaN and packed row-scale `I2_S` still needs a stable type and writer |
