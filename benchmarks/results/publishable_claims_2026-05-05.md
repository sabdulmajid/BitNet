...
| Direct packed row-scale GGUF export is still unsupported | supported | `benchmarks/results/direct_packed_gguf_support_2026-05-13.md`; `benchmark_results/direct_packed_gguf_support_2026-05-13.json` | Python direct-writer stack lacks `I2_S` constants/layout support and row-scale needs a stable type/version |
...
| Direct packed `ternary_state_dict.pt` to CPU-native GGUF writing is complete | unsupported | direct dense F16 GGUF now works, but packed row-scale `I2_S` still needs a stable type and writer |
